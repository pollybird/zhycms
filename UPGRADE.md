...
本文件包含各版本升级迁移指引。**v2.6.3 → v2.6.4 需执行迁移 `0008`（columns.member_only），无新增依赖**；**v2.6.2 → v2.6.3 无破坏性变更，覆盖代码 + 装依赖即可**；**v2.6.1 → v2.6.2 无破坏性变更，覆盖代码重启即可**；**v2.6.0 → v2.6.1 无破坏性变更，覆盖代码重启即可**；**v2.5.2 → v2.6.0 无破坏性变更，覆盖代码重启即可**；**v2.5.x → v2.5.2 自动迁移 `0007`**；**v2.4.x → v2.5.0 执行 Alembic 迁移 0005+0006 建翻译表，覆盖代码 + 装依赖即可**；运行 v1.1（及更早 1.x）版本的站点升级到 v2.0 涉及大量结构变更，请完整阅读本文后半部分后再操作。

---

## v2.6.3 → v2.6.4 升级（2026-09-12）

v2.6.4 是**前台会员体系 + 插件依赖治理**版本：新增社区插件「前台用户中心」（member），核心新增 `Column.member_only` 字段与前台访问守卫，插件系统新增依赖/继承/最低版本强校验。**数据库结构变更**：新增迁移 `0008`（给 `columns` 表补 `member_only` 列，默认 `FALSE`），member 插件首次启用时自动建三张会员表。**无新增 Python 依赖**。
...
# 0. 备份站点目录与数据库
# 1. 停服并更新代码到 v2.6.4
git pull    # 或下载 v2.6.4 发布包覆盖

# 2. 无新增依赖，启动服务（首次启动自动执行迁移 0008）
python run.py
```

> 首次启动自动执行 Alembic 迁移 `0008`，给 `columns` 表补 `member_only` 列（默认 `FALSE`，存量栏目不受影响）。

### 二、行为变化与兼容性

| 项目 | v2.6.3 | v2.6.4 | 升级影响 |
| --- | --- | --- | --- |
| 数据库结构 | 0007 | 0008（`columns.member_only`） | **自动迁移**，存量栏目 `member_only=FALSE` |
| 前台导航 | 无会员菜单 | navbar 右侧新增会员用户菜单（member 插件启用时） | 未启用 member 插件时输出空串，不影响现有页面 |
| 插件启用 | 仅检查加载成功 | 新增 min_core_version / requires / extends 强校验 | 现有插件未声明新字段，校验全部跳过，行为不变 |
| 插件禁用 | 直接移除 | 新增反向依赖校验（被依赖插件不可禁用） | 无依赖关系的插件不受影响 |
| `CMS_VERSION` | `2.6.3` | `2.6.4` | 后台页脚版本号自动更新 |
| 新增依赖 | — | 无 | — |

### 三、启用前台会员体系（可选）

不启用 member 插件的站点行为与 v2.6.3 完全一致。如需启用：

1. 后台 **插件管理** → 启用 `member`（首次启用自动建 `members` / `member_oauths` / `member_sms_codes` 三张表）。
2. 后台 **前台用户中心 → 插件设置**：
   - **基础设置**：注册开关、最小密码长度、注册协议（富文本编辑器）。
   - **短信通道**：默认「日志通道」（验证码写入应用日志，用于测试）；生产环境切换「阿里云短信」或「通用 Webhook」并填写凭证。
   - **微信 / QQ**：填写 AppID / AppSecret 后开启一键登录。
3. 后台 **前台用户中心 → 栏目可见性**：勾选需会员登录才能访问的栏目（树形批量操作）。
4. 前台右上角出现「登录 / 注册」按钮，会员登录后显示头像下拉菜单。

> 未登录用户访问 `member_only` 栏目及其文章会被 302 重定向到登录页；已登录会员可正常访问。

### 四、插件依赖校验（可选）

现有插件无需任何改动。自定义插件如需声明依赖关系，在 `manifest.json` 中添加：
...
启用时系统按「最低核心版本 → requires → extends」顺序强校验，不满足则拒绝启用并给出明确错误；禁用被依赖的插件也会被拦截。详见 `PLUGIN_DEPENDENCIES.md`。

### 五、升级后验证清单

- [ ] 应用启动日志出现 Alembic 迁移信息（`0008`），无报错
- [ ] 后台页脚显示版本号 `2.6.4`
- [ ] 前台 8 套主题首页正常显示，navbar 右侧无异常（未启用 member 时无用户菜单）
- [ ] 后台栏目编辑页可见「仅会员可见」开关（启用 member 后）
- [ ] 后台插件管理列表页可见各插件的最低版本/依赖/父插件徽章
- [ ] （可选）启用 member 后，前台右上角出现「登录/注册」，注册并登录后显示头像下拉菜单
- [ ] （可选）`flask db current` 显示当前版本为 `0008`

---

## v2.6.2 → v2.6.3 升级（2026-09-12）

v2.6.3 是**API 鉴权升级 + 行业模板扩充**版本：REST API 鉴权升级为 JWT（access/refresh 双 token），支持 token/jwt/both 双轨兼容，引入 flask-limiter 速率限制；新增教育（亮色调）与餐饮（暖色调）两套行业主题及配套演示数据。**无数据库结构变更**，新增 2 个依赖（flask-jwt-extended、flask-limiter），覆盖代码 + 装依赖即可升级。
...
- **JWT 密钥**：升级后 `jwt_secret_key` 为空，默认复用 `SECRET_KEY`。建议在后台「系统设置 → API 设置」中配置独立的 JWT 密钥（≥32 字节）以提升安全性。
- **鉴权模式**：默认 `api_auth_mode = both`，即同时支持旧的 `X-API-Token` 和新的 JWT `Authorization: Bearer`，**现有 API 调用不受影响**。如需强制使用 JWT，可在后台将鉴权模式改为 `jwt`。
- **速率限制**：默认开启，登录端点 5 次/分钟、只读 GET 端点 120 次/分钟。如遇误拦截，可在后台关闭限流或调大阈值。
- **新增行业主题**：education（教育/亮色调）、catering（餐饮/暖色调），可在后台「外观 → 主题」中切换，或重新运行 setup 向导生成对应演示数据。
- **旧 Token 鉴权完全保留**：`X-API-Token` 方式在 both/token 模式下继续可用，无需修改现有 API 客户端。

---

## v2.6.1 → v2.6.2 升级（2026-09-11）

v2.6.2 是**质量加固**版本：新增 5 大测试域 79 项测试（RBAC 权限边界 / 插件全生命周期 / Alembic 迁移 / API 契约 / 存储驱动抽象），测试规模 34 → 113 项；`oss_storage` 存储驱动抽象升级（驱动自动发现、`save_bytes` 字节流上传契约、驱动按归属插件门控回退本地）。**无数据库结构变更、无新增依赖**，覆盖代码重启即可。
...
- 若自行二开过 `app/utils/storage.py` 的 `register_driver`：签名新增可选参数 `owner_slug`（归属插件 slug，默认 `None` 不受门控），原调用无需修改。
- 若自定义插件注册了存储驱动：建议在 `get_storage_drivers` 返回的驱动上确保 `name` 属性唯一非空（自动发现契约），禁用插件后其驱动自动回退本地。
- `oss_storage` 插件版本升至 1.1.0，配置与数据完全兼容，无需迁移。

---

## v2.6.0 → v2.6.1 升级（2026-09-10）

v2.6.1 是**架构与可维护性**版本：蓝图注册抽离到 `app/blueprints.py`、admin 按 `content/` 与 `system/` 子包重组、新增 `app/services/` 业务逻辑层（article/column/user）、插件启用清单加 60s 缓存。**无数据库结构变更、无新增依赖**，覆盖代码重启即可。
...
- 纯代码重构，endpoint 命名空间（`admin.*`）不变，模板与 URL 不受影响。
- 若自定义了 `app/admin/*.py` 的导入路径，需调整为 `app/admin/content/*.py` 或 `app/admin/system/*.py`。
