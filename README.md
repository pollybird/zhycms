...
**当前版本：v2.2.0**（v2.0 八大能力之上新增插件优先架构、轮播图插件、产品增强模型插件、REST 内容 API、主题管理与插件上传/卸载/打包下载，详见 [v2.2.0 更新内容](#v220-更新内容)；历史版本见 [v2.1.1 更新内容](#v211-更新内容)、[v2.1.0 更新内容](#v210-更新内容) 与 [v2.0 更新亮点](#v20-更新亮点)）。**从 v1.1 升级请先阅读 [UPGRADE.md 升级迁移指南](UPGRADE.md) 并执行 `scripts/upgrade_v2.py`。**
...
- **插件机制**（v2.2）：`plugins/<slug>/` 目录零侵入扩展，启停即时生效、无需重启、禁用即隐身，内置轮播图、产品展示与友情链接三个官方插件作为开发样板。
...
zhycms/
├── app/
│   ├── admin/              # 后台模块（栏目、文章、表单、设置、用户、角色、审计、
│   │                       #   备份、插件管理、主题管理、内容 API 等）
│   ├── frontend/           # 前台模块
│   │   └── templates/
